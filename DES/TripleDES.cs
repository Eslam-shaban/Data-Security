...
namespace SecurityLibrary.DES
{
    ...
    public class TripleDES : ICryptographicTechnique<string, List<string>>
    {
        public string DesDecrypt(string cipherText, string key)
        {
            ...
            List<string> Key_s = new List<string>();
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                for (int j = 0; j < 8; j++)
                {
                    for (int k = 0; k < 6; k++)
                    {
                        EDIT = EDIT + r[EB[j, k] - 1];
                    }
                }

                for (int g = 0; g < EDIT.Length; g++)
                {
                    E_XOR_k = E_XOR_k + (N_keys[N_keys.Count - 1 - i][g] ^ EDIT[g]).ToString();
                }
        ...
        public string DesEncrypt(string plainText, string key)
        {
            ...
            for (int i = 0; i < 16; i++)
            {
                ...
                for (int g = 0; g < EDIT.Length; g++)
                {
                    E_XOR_k = E_XOR_k + (N_keys[i][g] ^ EDIT[g]).ToString();
                }
